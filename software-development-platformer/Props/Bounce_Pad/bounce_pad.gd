...
@export var JUMP_MULTIPLIER : float = 1.7
@export var JUMP_MAXIMUM : float = 1000.0
...
func _return_new_player_velocity(p_velocity: Vector2) -> Vector2:
	print("Initial player velocity: " + str(p_velocity))
	...
	normal_velocity *= -clamp(normal_length, JUMP_MINIMUM, JUMP_MAXIMUM) #keep jump in range
...
func _on_body_entered(body: Node2D) -> void:
	if body is Player:
		...
		print(body.velocity)
		#body.jump(_return_new_player_velocity(body.velocity), false)
		print(body.velocity)
