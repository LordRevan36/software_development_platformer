...
func _on_landed(landingVelocity: Vector2, collision: Object) -> void:
	if collision.get_collider().is_in_group("Bouncy") and landingVelocity.y > 200:
		bounceBody(landingVelocity.bounce(collision.get_normal()))
	else:
		if state == "backflip" or state == "frontflip":
			state = "flipland"
		elif state != "attack":
...
#function used by bouncy platforms and bouncy areas.
func bounceBody(bounceVelocity) -> void:
	state = "jump"
	canJump = false
	velocity.y = bounceVelocity.y*0.77 #not quite 1 because I made gravity faster when falling.
	velocity.x = bounceVelocity.x*1.1
	position.y+=1
